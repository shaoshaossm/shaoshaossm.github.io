...
function pvChart () { 
    ...
    fetch('/census/data/pv.json?date'+new Date()).then(data => data.json()).then(data => {
        ...
        // 扩展到2028年
        for (let i = 2020; i <= 2028; i++)   monthValueArr[String(i)] = [ ,  ,  ,  ,  ,  ,  ,  ,  ,  ,  ,  ];
        ...
        // 动态生成 legend 和 series 数据
        let legendData = [];
        let seriesData = [];
        let years = [2022, 2023, 2024, 2025, 2026, 2027, 2028];
        let colors = ['#01C2F9', '#18D070', '#d223e7', '#3F77FE', '#FF6B6B', '#4ECDC4', '#FFE66D'];
        
        for (let idx = 0; idx < years.length; idx++) {
            let year = years[idx];
            if (monthValueArr[String(year)] && monthValueArr[String(year)].some(v => v !== undefined && v !== null)) {
                legendData.push(year + '年访问量');
                seriesData.push(`{
                    name: '${year}年访问量',
                    type: 'line',
                    stack: '总量',
                    data: [${monthValueArr[String(year)]}],
                    axisLabel: {
                        formatter: '{value}',
                        textStyle: {
                            color: '#929298'
        ...
        var pvOption = {
    color: ${JSON.stringify(colors.slice(0, legendData.length))},
    title: {
        text: '站点访问量统计',
...
function uvChart () { 
    ...
    fetch('/census/data/uv.json?date'+new Date()).then(data => data.json()).then(data => {
        ...
        let seriesData = [];
        let years = [2022, 2023, 2024, 2025, 2026, 2027, 2028];
        let colors = ['#d223e7', '#3F77FE', '#01C2F9', '#18D070', '#FF6B6B', '#4ECDC4', '#FFE66D'];
        
        for (let idx = 0; idx < years.length; idx++) {
            let year = years[idx];
            if (monthValueArr[String(year)] && monthValueArr[String(year)].some(v => v !== undefined && v !== null)) {
                legendData.push(year + '年访客数');
                seriesData.push(`{
                    name: '${year}年访客数',
                    type: 'line',
                    smooth: true,
